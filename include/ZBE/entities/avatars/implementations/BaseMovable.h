...
class BaseMovable {
  public:
    ...
    void setPosition(std::initializer_list<double> l) {m->setPosition(x, y);}
    void setPosition(Point<s> position)   {m->setPosition(position);}

    void setVelocity(std::initializer_list<double> l) {m->setVelocity(x, y);}
    void setVelocity(Vector<s> velocity)  {m->setVelocity(velocity);}

    Point<s>& getPosition()  {return (m->getPosition());}
    Vector2D<s>& getVelocity() {return (m->getVelocity());}
