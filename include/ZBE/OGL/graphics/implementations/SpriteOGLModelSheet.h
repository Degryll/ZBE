 ...
 */
class SpriteOGLModelSheet : public OGLModelSheet<uint64_t, Vector2D, Vector2D> {
public:
  ...
  */
  ...
  OGLModel generateModel(std::shared_ptr<MAvatar<uint64_t, Vector2D, Vector2D> > avatar) {
    Vector2D pos = avatar->get<1, Vector2D>()->get();
    ...
    glm::vec3 s2{size.x * spriteDefintion.scale.x, size.y * spriteDefintion.scale.y, 1.0};
  ...
  void setSprite(SprtDef sd){
      spriteDefintion = sd;
      textures[0] = window->getTextureStore()->getTexture(spriteDefintion.img.imgSrcId);
  }
