...
#ifndef ZBE_FACTORIES_IMPLEMENTATIONS_DAEMONS_STATEMACHINEDMNFTRY_H_
#define ZBE_FACTORIES_IMPLEMENTATIONS_DAEMONS_STATEMACHINEDMNFTRY_H_
...
/** \brief Factory for State Machine Daemons.
 */
class StateMachineDmnFtry : virtual public Factory {
public:
  ...
  void setup(std::string name, uint64_t cfgId) override {
    ...
    if(cfg) {
      ...
      for (auto& stateCfg : statesCfg.items()) {
        ...
        if(auto dmn = JSONFactory::StoreLoader<Daemon>::loadParamStrStore(daemonRsrc, zbe::factories::daemonName_ + stateCfg.value().get<std::string>() , "StateMachineDaemonFtry"s)) {
...
#endif  // ZBE_FACTORIES_IMPLEMENTATIONS_DAEMONS_STATEMACHINEDMNFTRY_H_
