 ...
 * @file KeyDownTicketEnablerIH.cpp
 * @since 2017-07-05
 * @date 2017-07-05
...
#include "ZBE/events/handlers/input/KeyDownTicketEnablerIH.h"
 ...
 void KeyDownTicketEnablerIHFtry::create(std::string name, uint64_t) {
   using namespace std::string_literals;

   auto ih = std::shared_ptr<KeyDownTicketEnablerIH>(new KeyDownTicketEnablerIH);
   ihRsrc.insert("InputHandler."s + name, ih);
   ttihRsrc.insert("KeyDownTicketEnablerIH."s + name, ih);
 }

 void KeyDownTicketEnablerIHFtry::setup(std::string name, uint64_t cfgId) {
   using namespace std::string_literals;
