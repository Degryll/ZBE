...
std::shared_ptr<zbe::Daemon> ZBNoidWinScreenBuilder::build() {
  ...
  auto drawerDaemon = std::make_shared<BehaviorDaemon<TicketedFAEC<AnimatedSprite>, AnimatedSprite > >(std::make_shared<SpriteSheetSDLDrawer<AnimatedSprite> >(window) , RsrcStore<TicketedFAEC<AnimatedSprite> >::getInstance().get(ZBNCfg::WIN_BUTTONS_AS_LIST));
  auto writerDaemon = std::make_shared<BehaviorDaemon<TicketedFAEC<SingleTextSprite>, SingleTextSprite > >(std::make_shared<SingleTextSDLDrawer>(window) , RsrcStore<TicketedFAEC<SingleTextSprite> >::getInstance().get(ZBNCfg::WIN_BUTTONS_TS_LIST));
