...
TEST(SysError, InitialState) {
  ...
  EXPECT_STREQ("",zbe::SysError::getFirstErrorString().c_str()) << "Checking error string is "".";
}
...
TEST(SysError, SetError) {
  ...
  EXPECT_STREQ("Test 1",zbe::SysError::getFirstErrorString().c_str()) << "Checking error string.";

  zbe::SysError::setError("Test 2");
  EXPECT_EQ(2,zbe::SysError::getNErrors()) << "Checking second error.";
  EXPECT_STREQ("Test 1",zbe::SysError::getFirstErrorString().c_str()) << "Checking that the error string does not change.";
  zbe::SysError::clear();
}
...
TEST(SysError, Clear) {
  ...
  EXPECT_STREQ("",zbe::SysError::getFirstErrorString().c_str()) << "Checking error string is "" again.";
}
...
TEST(SysError, Complete) {
  ...
  EXPECT_STREQ("Test 1",zbe::SysError::getFirstErrorString().c_str())  << "Checking that error string is the string of the first error.";
