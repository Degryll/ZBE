...
#include "zombienoid/entities/avatars/implementations/PositionableTimerSprite.h"
#include "zombienoid/events/handlers/BallExploder.h"
 ...
 public:
  ...
  void act(std::shared_ptr<zbe::WeakAvatarEntityContainer<zombienoid::Boombizer> >) {
    ...
    if(stated->getState() == boombState) {
      ...
      auto tsList = zbe::ResourceManager<TSL>::getInstance().get(tsListId);
